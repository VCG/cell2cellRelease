...
void main() {
        ...
        vec3 new_pos = start_loc + clip_plane;
        float val = sample1(new_pos);
        vec3 dstep = 1.5 / u_size;
        gl_FragColor = add_lighting(val, new_pos, dstep, view_ray)/2.0;
/*
        val = sample2(new_pos);
        gl_FragColor += add_lighting2(val, new_pos, dstep, view_ray);

        val = sample3(new_pos);
        gl_FragColor += add_lighting3(val, new_pos, dstep, view_ray);
*/
        if(clip_plane.z < -1.0) {
