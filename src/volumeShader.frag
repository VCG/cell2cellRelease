...
varying vec3 worldSpaceCoords; // front faces, xyz positions
...
void main() {
        ...
        if (gl_FragColor.a < 0.05)
                discard;
      //  gl_FragColor = vec4(gl_FragDepth,gl_FragDepth,gl_FragDepth,0.5)
     //  return;
}
...
void cast_dvr(vec3 start_loc, vec3 step, int nsteps, vec3 view_ray) {
        ...
        for (int iter=0; iter<MAX_STEPS; iter++) {
                ...
                if (readDepth( u_geo_depth, vUv ) > gl_FragCoord.z) {
                ...
                // Advance location deeper into the volume
                loc += step;
                gl_FragDepth = distance(loc.z,gl_FragCoord.z);
        }
}
...
float readDepth( sampler2D depthSampler, vec2 coord ) {
        ...
        //float viewZ = perspectiveDepthToViewZ( fragCoordZ, u_camera_near, u_camera_far );
        return viewZToOrthographicDepth( fragCoordZ, u_camera_near, u_camera_far );
}